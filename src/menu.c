...
struct menu *menu_initialize(SDL_Renderer *renderer) {
    ...
    menu->selected_difficulty = PLAY_MEDIUM; // sensible default if the player never touches it
...
void menu_run(struct menu *menu) {
    ...
    while (menu->state != MENU_PLAY && menu->state != MENU_QUIT) {
            ...
            if (e.type == SDL_QUIT) {
                ...
                switch (e.key.keysym.sym) {
                    ...
                    case SDLK_DOWN:
                        ...
                        break;
                    case SDLK_RETURN:
                        // FIX: on Easy/Medium/Hard, Enter now actually confirms
                        // that difficulty instead of just cycling focus to the
                        // next one (which meant the choice was never recorded
                        // and never reached play_initialize).
                        if (menu->state == MENU_PLAY_FOCUS) {
                            menu->state = MENU_PLAY;
                        } else if (menu->state == MENU_QUIT_FOCUS) {
                            menu->state = MENU_QUIT;
                        } else if (menu->state == MENU_EASY_FOCUS) {
                            menu->selected_difficulty = PLAY_EASY;
                        } else if (menu->state == MENU_MEDIUM_FOCUS) {
                            menu->selected_difficulty = PLAY_MEDIUM;
                        } else if (menu->state == MENU_HARD_FOCUS) {
                            menu->selected_difficulty = PLAY_HARD;
                        }
                        break;
                }
            }
        ...
        // Highlighted if the arrow keys are on it right now, OR if it's the
        // difficulty actually confirmed with Enter — otherwise a selected
        // difficulty looks "unselected" as soon as focus moves away.
        spritesheet_render(menu->easy, left_margin, EASY_Y - 10,
                           menu_alpha(menu->state == MENU_EASY_FOCUS ||
                                      menu->selected_difficulty == PLAY_EASY), 0);
        spritesheet_render(menu->medium, left_margin + spacing, MEDIUM_Y - 10,
                           menu_alpha(menu->state == MENU_MEDIUM_FOCUS ||
                                      menu->selected_difficulty == PLAY_MEDIUM), 0);
        spritesheet_render(menu->hard, left_margin + (2 * spacing), HARD_Y - 10,
                           menu_alpha(menu->state == MENU_HARD_FOCUS ||
                                      menu->selected_difficulty == PLAY_HARD), 0);
